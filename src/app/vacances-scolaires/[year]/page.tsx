  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <p className="text-text-secondary mt-2 max-w-xl">
        Année scolaire du {formatLongDateFR(data.startDate)} au {formatLongDateFR(data.endDate)}.
      </p>
      {data.source && (
        <p className="text-xs text-text-secondary mt-1">Source : {data.source}</p>
      )}
      ...
      <section className="mt-8">
        ...
        <ul className="space-y-2">
          {data.terms.map((term) => (
            ...
            >
              <span className="font-medium text-text-primary">{term.label}</span>
          ...
          {data.breaks.map((b) => (
            ...
            >
              ...
              {b.status === "provisional" && (
                <p className="flex items-center gap-1.5 text-xs mt-1 text-primary-text">
